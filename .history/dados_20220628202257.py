...
imagem = cv2.imread("img/dados4.jpg")          # Carregar a imagem
...
ret,binary = cv2.threshold(gray , 85 ,255, cv2.THRESH_BINARY)         # Binarização ################
ret1,binary1 = cv2.threshold(binary,127,255,cv2.THRESH_BINARY_INV)   # Inverter a Binarização

kernel = np.ones((6,6), np.uint8)
dilation = cv2.dilate(binary1, kernel, iterations=1)   # Dilatar ####################
