...
imagem = cv2.imread("imgDados/dadosFundo.jpg")          # Carregar a imagem
gray = cv2.cvtColor(imagem, cv2.COLOR_BGR2GRAY)     # GrayScale
blur = cv2.blur(gray,(3,3))                       # Blur ###############

ret,binary = cv2.threshold(blur , 90 ,255, cv2.THRESH_BINARY)         # Binarização ################
